...
`harbor-tool-diff-mark` is centered on cli tools work in Python. The technical target is: Package a Python local lab for diff analysis with windowed input fixtures, late-data behavior checks, and documented operating limits.
...
The score is deterministic. The current threshold is 180. Risk has a penalty of 5, latency has a penalty of 2, and weight contributes a bonus of 2.

...
harbor-tool-diff-mark is a Python project for cli tools. It focuses on this technical goal: Package a Python local lab for diff analysis with windowed input fixtures, late-data behavior checks, and documented operating limits.
...
The core module exposes a small scoring API. Inputs are simple numeric signals: demand, capacity, latency, risk, and weight. The score uses a threshold of 180, risk penalty 5, latency penalty 2, and weight bonus 2. Tests exercise the public API against the fixture cases in `fixtures/cases.csv`.
